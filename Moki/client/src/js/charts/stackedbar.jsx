...
*/
export default function StackedChart({ data, id, width, name, units, keys }) {
  useEffect(() => {
    draw();
  }, [data, width]);
  ...
  const draw = () => {
    ...
    if (data === undefined || data.length === 0) {
      ...
      keys = store.getState().persistent.layout.types[keys];
      ...
      layer.selectAll("rect")
        ...
        .on("mouseover", function (event, d) {
          tooltip.select("div").html(
            "<strong>Type: </strong> " + this.parentNode.getAttribute("type") +
              "<br/><strong>Count:</strong> " + d3.format(",")(d[1] - d[0]) +
              units + "<br/>",
          );
  ...
  return (
    <div id={id} className="chart chartMinHeight">
      <h3 className="alignLeft title" style={{ "float": "inherit" }}>{name}</h3>
    </div>
  );
}
