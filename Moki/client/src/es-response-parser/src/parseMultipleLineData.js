...
export default function parseMultipleLineData(response) {
  if (
    response.aggregations && response.aggregations.agg &&
    response.aggregations.agg.buckets && response.aggregations.agg.buckets[0] &&
    response.aggregations.agg.buckets[0][2]
  ) {
